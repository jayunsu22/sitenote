...
- **현장 달력** (`MonthWidget`): 한 달 날짜별 시공 지역과 빈 날 — 새 일·AS 잡을 때
...
- 일정 규칙(`Schedule.kt`·`Month.kt`)은 웹앱 `share.js`·`app.js` 를 그대로 옮긴 것이다
  (달력 지역은 `autoRegion`·`regionOf`·`dateRegions`·`monthGridFull`).
...
- 위젯 그림(에뮬레이터 없이): `RENDER_PREVIEW=1 PREVIEW_JSON=… PREVIEW_OUT=out.png` (달력은 `PREVIEW_KIND=month`)
